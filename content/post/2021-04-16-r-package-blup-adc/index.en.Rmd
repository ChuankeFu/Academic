...
用户提供的 Plink-ped格式的数据，`data.frame`类型。具体格式如下所示：
...
用户提供的 Plink-map格式的数据，`data.frame`类型。具体格式如下所示：
...
用户提供的 Hapmap格式的数据，`data.frame`类型。具体格式如下所示
...
用户提供的 BLUPF90格式数据，`data.frame`类型。具体格式如下所示
...
用户提供的 Numeric格式的数据，`matrix(纯数字)`类型。具体格式如下所示
...
此外，为了方便用户使用，用户还可以直接通过提供本地数据的路径、名称与数据类型即可完成数据提供这一步骤,而无需将数据读入到**R**里面。
...
-   **参数8:input_data_type**
    ...
    -   Blupf90
    -   Numeric
    -   VCF(需要提前安装好vcftools软件,并设置好环境变量)
